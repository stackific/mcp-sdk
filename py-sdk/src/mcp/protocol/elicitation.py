...
def is_valid_requested_schema(value: object) -> bool:
  ...
  if "required" in value:
    req = value["required"]
    if not isinstance(req, list) or not all(isinstance(s, str) for s in req):
      return False
  return "$schema" not in value or isinstance(value["$schema"], str)
...
def is_valid_form_params(value: object) -> bool:
  ...
  if not isinstance(value, dict):
    return False
  if "mode" in value and value["mode"] != ELICITATION_MODE_FORM:
    return False
  return isinstance(value.get("message"), str) and is_valid_requested_schema(value.get("requestedSchema"))
...
def is_valid_url_params(value: object) -> bool:
  ...
  """
  if not isinstance(value, dict) or value.get("mode") != ELICITATION_MODE_URL:
    return False
  return (
    isinstance(value.get("message"), str)
    and isinstance(value.get("elicitationId"), str)
    and value["elicitationId"] != ""
    and is_valid_elicitation_url(value.get("url"))
  )
